...
const SUPABASE_URL = (typeof process !== 'undefined' && process.env.VITE_SUPABASE_URL) || 'https://aumqjpqngmhpbwytpets.supabase.co';
const SUPABASE_ANON_KEY = (typeof process !== 'undefined' && process.env.VITE_SUPABASE_ANON_KEY) || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF1bXFqcHFuZ21ocGJ3eXRwZXRzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxODgyNjMsImV4cCI6MjA4ODc2NDI2M30.OHi4bRiyFUv2lBHu3wb1IKchj2qF6rZ354uhCQeeAlU';
...
export const AdminPropertiesList = () => {
    ...
    try {
      ...
      for (const platform of platformsToPublish) {
        ...
        const res = await fetch(
          `${SUPABASE_URL}/functions/v1/share-to-community`,
          {
            method: 'POST',
            headers: {
  ...
  const handleIdealistaSync = async (p: Property) => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(
        `${SUPABASE_URL}/functions/v1/idealista-sync`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleOpenImportModal = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(
        `${SUPABASE_URL}/functions/v1/idealista-import`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
