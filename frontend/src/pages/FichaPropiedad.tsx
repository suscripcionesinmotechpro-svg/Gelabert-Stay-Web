        ...
        {activeTab === 'fotos' && (
          <div className="flex flex-col gap-6">
            ...
            {generalImages.length > 0 ? (
              <>
                <PremiumImage 
                  src={getOptimizedImage(generalImages[activeImg], { width: 1920, height: 1080, quality: 100, format: 'webp' })} 
                  alt={translatedTitle} 
          ...
          {generalImages.length > 1 && (
            <div className="flex md:flex-col gap-2 md:w-1/3 overflow-x-auto md:overflow-y-auto md:max-h-[600px]">
              {generalImages.map((img, i) => (
                ...
                >
                  <PremiumImage src={getOptimizedImage(img, { width: 800, height: 600, quality: 100, format: 'webp' })} alt={`${i}`} wrapperClassName="w-full h-full" />
                </button>
              ))}
            </div>
          )}
