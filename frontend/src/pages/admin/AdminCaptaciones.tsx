...
export const AdminCaptaciones = () => {
  ...
  // Fetch all captaciones or agent-specific
  ...
  const handleAssignAgentDirectly = async (agentId: string | null) => {
    if (!selectedCaptacion) return;
    try {
      await updateCaptacion(selectedCaptacion.id, { agent_id: agentId });
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredCaptaciones.map(c => [
      ...
      escapeCSV(getAgentName(c.agent_id)),
    ...
    link.setAttribute('download', `Captaciones_${periodLabel}_${dateLabel}.csv`);
  ...
  const handleExportPDF = () => {
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Reporte de Captaciones - ${periodLabel}</title>
        <style>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      {/* Top Header */}
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4 bg-[#111111] p-6 border border-[#1F1F1F] rounded-lg">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#161616] text-[#FAF8F5] border border-[#1F1F1F] font-primary font-bold text-xs uppercase tracking-wider hover:bg-[#1F1F1F] transition-colors whitespace-nowrap"
            title="Imprimir / Guardar reporte PDF de la lista actual"
          >
                  ...
                  {!isEditing ? (
                    <>
                      ...
                      <div className="flex items-center gap-1.5 bg-[#111] border border-[#1F1F1F] px-2.5 py-1 text-xs">
                        ...
                        <select
                          value={selectedCaptacion.agent_id || ''}
                          onChange={(e) => handleAssignAgentDirectly(e.target.value || null)}
