...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  if (!post) return { title: 'Article | Gelabert Homes' };

  const title = post.title_en || post.title;
  const desc = post.excerpt_en || post.excerpt;

  return {
    title: `${title} | Gelabert Homes`,
    description: desc || 'Article from our real estate blog.',
    openGraph: {
      title: title,
      description: desc,
